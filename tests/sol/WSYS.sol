...
contract WSYS is StandardToken
{
	...
	function wrap() public payable returns (bool _success)
	{
		address _from = msg.sender;
		uint256 _value = msg.value;
		require(_value > 0);
		require(supply + _value > supply);
		assert(balances[_from] + _value > balances[_from]);
		supply += _value;
		balances[_from] += _value;
		return true;
	}

	function unwrap(uint256 _value) public returns (bool _success)
	{
